...
Per `Items._of` and the validate-check walk in `items.py`, a var listed in this note's frontmatter that points at a `.py` file must be in `path::item_name` form, because `_of` returns `None` for whole-file refs whose extension is in any `Lang.exts` set (forcing the validate-check to report "dep does not exist, break down to items"). Hence the four `path::item` entries above instead of two file paths. Validation requires each cited item to carry a Python docstring (first body statement is a triple-quoted string); `/validate-mark path/to/file.py::name` produces the docstring by structurally upgrading a preceding `#` comment run (per `skills/validate-mark/lang/python.md` Case A / B / C and `hooks/post_skill_trigger.py::PostMark::_python_upgrade`).
